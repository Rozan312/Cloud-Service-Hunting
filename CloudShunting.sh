...
#Emeration domain cloud from JavaScript
...
#Permutation enumerate cloud service with cloud_enum from namedomain to key product.txt
cat $domain/alive.txt | grep -oP "(?<=://)[^./\s]+(?=\.$domain)" | sort -u | anew $domain/product.txt #use this (#) if you don't want to use cloud enum

#Permutation enumerate cloud service with cloud_enum 
...
echo =====Parsing Cloud Service Only====
